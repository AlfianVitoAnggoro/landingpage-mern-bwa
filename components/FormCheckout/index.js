...
export default function FormCheckout({ tickets }) {
  ...
  useEffect(() => {
    const fetctData = async () => {
      try {
        const res = await getData(`api/v1/payments/${organizer}`, {}, Cookies.get('token'));
  ...
  const handleSubmit = async () => {
    try {
      ...
      const res = await postData('api/v1/checkout', payload, Cookies.get('token'));
      ...
      <div className="personal-details">
        ...
        <div className="row row-cols-lg-8 row-cols-md-2 row-cols-1 justify-content-center">
          <div className="mb-4 col-lg-4">
            ...
            <input type="text" placeholder="First name here" className="form-control" id="first_name" name="firstName" value={form.firstName} onChange={handleChange} />
          </div>
          ...
          <div className="mb-4 col-lg-4">
            ...
            <input type="text" placeholder="Last name here" className="form-control" name="lastName" id="last_name" value={form.lastName} onChange={handleChange} />
          </div>
        </div>
            ...
            <div className="col-lg-4" key={payment._id}>
              <label className="payment-radio h-100 d-flex justify-content-between align-items-center">
                <div className="d-flex align-items-center gap-4">
                  <img src={`${process.env.NEXT_PUBLIC_API_IMAGE}/${payment.imageUrl}`} alt="" />
                  <div>{payment.type}</div>
                </div>
                <input type="radio" checked={payment.isChecked} name="isChecked" value={payment._id} onChange={(e) => handleChangePayment(e, i)} />
